...
module.exports = function(config) {
    ...
    app.get('/__zuul/executor', function(req, res) {
        res.locals.config = { port: config.support_port };
        res.render('executor');
    });
    ...
    var controlBundle = browserify(opt);
    controlBundle.require(require.resolve('../frameworks/controller.js'), { entry: true });
    controlBundle = watchify(controlBundle);

    bundle_router.get('/__zuul/controller.js', function(req, res, next) {
        res.contentType('application/javascript');

        var start = Date.now();
        controlBundle.bundle(function(err, buf) {
            if (err) {
                return next(err);
            }
